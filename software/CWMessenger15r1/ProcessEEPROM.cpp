...
#ifdef DEBUG
  Serial.print("UpdateSwitchesArray: ");
  Serial.print(switchIndexes[0]);
...
void ReadSwitchesArray(void)
{
  ...
  if ((switchIndexes[0] > MAXMESSAGES) || (switchIndexes[0] < 1) ||
      (switchIndexes[1] > MAXMESSAGES) || (switchIndexes[1] < 1) ||
      (switchIndexes[2] > MAXMESSAGES) || (switchIndexes[2] < 1)) {
    switchIndexes[0] = 1;
    switchIndexes[1] = 2;
    switchIndexes[2] = 3;
    UpdateSwitchesArray();
...
int ReadWordsPerMinute(void)
{
  ...
  if (wordsPerMinute > MINWPM && wordsPerMinute < MAXWPM) {
    ...
    //WriteWordsPerMinute();
    // Don't really need this.  If it isn't updated, will go through
    // the same stuff next time.  Namely will use default MESSAGEWPM
    //WriteWordsPerMinute();
  }
  return wordsPerMinute;
}
